...
async function deleteDomainEmails(domain) {
  ...
  try {
    ...
    state.emails = state.emails.filter(
      (e) => e.domain !== domain || failIds.has(e.id)
    );
    ...
    if (ok > 0)
      toast(`Deleted ${ok} email${ok === 1 ? "" : "s"} from ${domain} · synced to Gmail`, "ok");
    if (failIds.size)
      toast(`${failIds.size} email${failIds.size === 1 ? "" : "s"} failed — kept in view.`, "err");
  } catch (err) {
...
function renderDomainList() {
  ...
  list?.querySelectorAll("[data-del-domain]").forEach((btn) =>
    btn.addEventListener("click", async (e) => {
      e.stopPropagation();
      await deleteDomainEmails(btn.dataset.delDomain);
    })
  );
}
...
function toast(msg, kind) {
  ...
  t.className = kind === "err" ? "toast toast-err" : kind === "ok" ? "toast toast-ok" : "toast";
  t.textContent = msg;
